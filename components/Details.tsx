  ...
  return (
    <>
      ...
      >
        ...
        <p className="text-[20px] font-[500px] text-center">{poll.description}</p>
        ...
        <div className=" h-[136px] gap-[16px] flex flex-col items-center mt-4">
          ...
          <div className="h-[36px] gap-[4px] flex justify-center items-center">
            ...
            <button
              className="py-[6px] px-[12px] border border-gray-400 
              bg-white bg-opacity-20 rounded-full text-[12px] md:text-[16px]"
            >
              {poll.contestants === 1 ? `${poll.contestants} contestant` : `${poll.contestants} contestants`}
            </button>
          ...
          {poll.votes < 1 && (
            <button
           //  disabled={wallet ? poll.contestants.includes(wallet) || Date.now() < poll.startsAt || Date.now() >= poll.endsAt : true}
